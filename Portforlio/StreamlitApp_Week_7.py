...
MODEL_INFO = {
        ...
        "keys": ["AMD", "NVDA"],
        "inputs": [{"name": k, "type": "number", "min": 0.0, "default": 0.0, "step": 10.0} for k in ["AMD", "NVDA"]]
}
...
def display_explanation(input_df, session, aws_bucket):
    ...
    preprocessing_pipeline = Pipleine(steps=best_pipeline.steps[:-2])  
    ...
    st.pyplot(fig)
    # top feature   
    top_feature = pd.Series(shap_values[0, :, 2].values, index=shap_values[0, :, 2].feature_names).abs().idmax()
